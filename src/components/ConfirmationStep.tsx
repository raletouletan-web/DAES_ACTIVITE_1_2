...
const ConfirmationStep: React.FC<Props> = ({ userInfo, questions }) => {
  ...
  const buildTextBody = useCallback(() => {
    ...
    questions.forEach((q, idx) => {
      ...
      if (q.banner) b += `${q.banner}\n`;
      b += `${q.question}\n\n`;
      b += `Réponse :\n${q.answer || '(aucune réponse)'}\n\n`;
    });
    b += `─────────────────────────────────────\n`;
    b += `Envoyé via EmailJS\n`;
    return b;
  ...
  const handleDownloadTxt = () => {
    ...
    a.href = url;
    a.download = `transcription-${userInfo.lastName}-${userInfo.firstName}.txt`;
    a.click();
  ...
  const handleDownloadJSON = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `transcription-${userInfo.lastName}-${userInfo.firstName}.json`;
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const handleClosePage = () => {
    ...
    setTimeout(() => {
      document.body.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:center;min-height:100vh;background:#f0f7ff;font-family:sans-serif;">
          <div style="text-align:center;padding:40px;">
            <p style="font-size:48px;margin-bottom:16px;">✅</p>
            <h2 style="font-size:20px;font-weight:bold;color:#1e293b;margin-bottom:8px;">Session terminée</h2>
            <p style="color:#64748b;font-size:14px;">Vous pouvez fermer cet onglet manuellement.</p>
          </div>
        </div>`;
    }, 500);
  };
        ...
        <div className="p-8 md:p-10">
          ...
          {status === 'sending' && (
            <div className="text-center py-8">
              ...
              <h3 className="text-xl font-semibold text-slate-900 mb-2">Envoi en cours…</h3>
              <p className="text-slate-600 mb-6">Transmission sécurisée via EmailJS</p>
              ...
              <div className="inline-flex items-center gap-6 text-sm text-slate-500 bg-slate-50 px-6 py-3 rounded-2xl">
                <span className="flex items-center gap-2"><Send className="w-4 h-4" />raletouletan@gmail.com</span>
                <span className="text-slate-300">•</span>
                <span className="flex items-center gap-2"><Send className="w-4 h-4" />{userInfo.email}</span>
              </div>
              {retryCount > 0 && <p className="text-xs text-amber-600 mt-4">Tentative {retryCount + 1}…</p>}
            </div>
          )}
          ...
          {status === 'success' && (
            <>
              <div className="text-center mb-8">
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-2">Email envoyé avec succès !</h3>
              ...
              <div className="grid sm:grid-cols-3 gap-3 mb-6">
                <button onClick={handleDownloadTxt} className="flex items-center justify-center gap-2 px-4 py-3 bg-slate-900 hover:bg-black text-white rounded-xl font-medium transition-colors">
                  <FileText className="w-4 h-4" /> Télécharger .TXT
                </button>
                <button onClick={handleDownloadJSON} className="flex items-center justify-center gap-2 px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl font-medium transition-colors">
                  <FileJson className="w-4 h-4" /> Télécharger .JSON
                </button>
                <button onClick={handleCopy} className="flex items-center justify-center gap-2 px-4 py-3 bg-white hover:bg-slate-50 text-slate-800 border border-slate-300 rounded-xl font-medium transition-colors">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3.5 bg-gradient-to-r from-slate-700 to-slate-800 hover:from-slate-800 hover:to-slate-900 text-white font-semibold rounded-2xl shadow-lg transition-all"
              >
          ...
          {status === 'error' && (
            <>
              <div className="text-center mb-8">
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-2">Échec de l'envoi</h3>
                <p className="text-slate-600 max-w-md mx-auto">L'email n'a pas pu être envoyé via EmailJS.</p>
              </div>
              ...
              <div className="bg-red-50 border border-red-200 rounded-2xl p-5 mb-6">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {errorCode && (
                      <p className="text-xs font-mono text-red-700 mt-2 bg-red-100 inline-block px-2 py-1 rounded">Code: {errorCode}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 mb-6">
                <h4 className="font-semibold text-amber-900 mb-2.5 flex items-center gap-2">
                  <Info className="w-4 h-4" /> Vérifications
                </h4>
